...
export class ModifyResponseProvider implements Provider<Send> {
  ...
  addHeaders(response: Response, result: OperationRetval) {
    ...
    if(!isInterNalRequest && !Buffer.isBuffer(result) && responseContentType && typeof responseContentType == 'string' && responseContentType.includes('application/json')){
	    result = JSON.parse(he.encode(JSON.stringify(result),{'useNamedReferences': true}).replace(/\&quot;/g,'"').replace(/\+/g,"&plus;").replace(/\(/g,"&lpar;").replace(/\)/g,"&rpar;"))
    }
